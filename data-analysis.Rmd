...
```{r eval}
t <- head(data[ which(data$rd.rp.stid==1), ])
t <- head(data[ which(data$rd.rp.stid==2), ])
t <- head(data[ which(data$rd.rp.stid==3), ])
t <- head(data[ which(data$rd.rp.stid==4), ])
t <- head(data[ which(data$rd.rp.stid==5), ])
t <- head(data[ which(data$rd.rp.stid==6), ])
t <- head(data[ which(data$rd.rp.stid==7), ])
t <- head(data[ which(data$rd.rp.stid==8), ])
t <- head(data[ which(data$rd.rp.stid==9), ])
t <- head(data[ which(data$rd.rp.stid==10), ])
```
### examine stid = NA
```{r}
...
```
### examine roads with most positive comments in general
```{r}
...
slices <- neg$sum.neg
lbls <- neg$V1
pct <- round(slices/sum(neg$sum.neg)*100)
lbls <- paste(lbls, pct) 
lbls <- paste(lbls,"%",sep="") 
